...
from cone_datasets import TrainDataset
...
def train_lstm_model(generator, model_save_dir, config, val_data=None, model_load=None):
    ...
    with strategy.scope():
        ...
        model.fit(generator, epochs=200, validation_data=val_data,
                  steps_per_epoch=1000,
                  callbacks=[epoch_callback, csv_logger])
...
def main():
    ...
    for d in [config_dict["model_save_dir"]]:
    ...
    # Init generator and val geneerator

    train_dataset_obj = TrainDataset(batch_size=64)
    generator = train_dataset_obj.create_dataset()

    val_dataset_obj = TrainDataset(batch_size=64, split='val', repeat=False)
    val_data = val_dataset_obj.create_dataset()
